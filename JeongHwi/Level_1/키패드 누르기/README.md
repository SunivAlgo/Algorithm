...
각 키패드를 2차원 배열로 좌표를 구성했고 빠른 접근을 위해서 dictionary로 구현했다.

`r_dist`는 현재 r_hand의 번호와 해당하는 번호 (2,5,8,0 중에 하나) 의 거리를 구한다.

y좌표, x좌표를 각각 빼서 `abs` (절댓값) 을 구하면 거리가 되고 `l_dist` 도 동일하게 구해준다.

거리가 같으면 입력받는 `hand` 매개변수에 해당하는 방향을 더한다.
