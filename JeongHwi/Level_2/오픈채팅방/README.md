...
database는 `id : name` 으로 이루어져 있는 Dictionary이다.

opQueue는 `Operation ID` 로 Queue에 들어간다.

database를 Dictionary로 선정한 이유는 `닉네임을 변경할 때는 기존에 채팅방에 출력되어 있던 메시지의 닉네임도 전부 변경된다.` 라는 조건 때문에 계속 바꿔줘야한다는 것이다.

전체 record를 다 돌면 그 뒤에 queue를 돌아서 명령을 수행한다.
