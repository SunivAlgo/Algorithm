...
[출처 - Programmers](https://programmers.co.kr/learn/courses/30/lessons/67257)
...
우선 정규표현식으로 숫자와 연산자를 구분해주고 priority가 높은 순으로 먼저 계산해서 스택에 쌓는다

예를 들어

```python
100-200*300-500+20 , priority = [* + -] 라고 가정하자.
st---
priority (*)
100
100 200
100 200 300 < 200과 300사이에 *가 있으므로 계산해서 다시 스택에 넣음
100 60000 500 20
numbers를 다 돌면, numbers=st 으로 해준다.
```

> 여기에 연산자 st도 마찬가지로 해주면 된다.

계산을 마치면 최종적으로 절댓값을 씌워 max값을 계산한다.

근데 내가 청므에 짯던 코드가 왜 틀린지 이해가 안된다.

너무 시간이 오래걸린 문제;; (무려 4시간..)
