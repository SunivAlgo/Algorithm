...
[출처 - Programmers](https://programmers.co.kr/learn/courses/30/lessons/17677)
...
`makeSub` 함수는 2 알파벳씩 자르는 함수다.

해당 문제에서는 union은 중복된 부분단어에 대해서 `max`값을 추가하고 , intersection은 `min`값을 추가한다.

따라서 `counter` 함수로 각 부분집합마다 갯수를 세주고, `min` , `max`로 값을 처리해 계산한다.



문제를 제대로 읽지 않아서 겁나 오래걸렸다.
