...
* HEAD랑 NUMBER를 나누는 것을 `while`로 수행했는데, 여기서 NUMBER를 만드는 과정에서 `abc123456789` 인 경우에 9까지 체크하다가 while True로 인하여 무한루프에 빠지게 된다. 하지만 문제에서는.. `다섯 개...` 라고 표현을 했다. 그래서 for문으로 고쳐주었더니 시간초과가 해결됨
* 이 문제에서 핵심은 무엇보다 HEAD NUMBER를 나누는게 핵심이었다.
  * TAIL은 의미없음
